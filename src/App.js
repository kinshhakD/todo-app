...
const App = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8000/lists?_expand=color&_embed=tasks')
      .then(({ data }) => {
        setLists(data);
      });
  ...
  const onEditTask = (listId, taskObj) => {
    const newTaskText = window.prompt('Название:', taskObj.text);
  ...
  return (
    <div className="todo">
      <div className="todo__sidebar">
        ...
        />
        {lists ? (<List items={lists} activeItem={activeItem} isRemovable onRemove={(id) => {
